...
@Module({
  imports: [
    ...
    GraphQLModule.forRoot({
      ...
      formatError: error => {
        if (error.name === "ForbiddenError") throw new ForbiddenError("권한이 없습니다.");
        if ("response" in error.extensions && error.extensions.response != null) {
          const response = error.extensions.response as { message: string | string[] };
          if (Array.isArray(response.message)) return { message: response.message.join("\n"), ...error };
